...
from retinanet.model import ModelWrapper, RetinaNet
...
def init_models(big_model_path, small_model_path):
    if str(big_model_path).endswith("retina_1014.pth"):
        big_model = RetinaNet(num_classes = 2)
        s_d = torch.load(big_model_path, map_location='cuda')
        big_model.load_state_dict(s_d, strict=True)
        big_model = ModelWrapper(big_model, img_shape = [1024,1024])
    else:
    ...
    # big_model = TRTModule()
    # big_model.load_state_dict(torch.load("weights/model_trt_fp16.pth"))
    # big_model = ModelWrapper(big_model, img_shape = [1024,1024])
